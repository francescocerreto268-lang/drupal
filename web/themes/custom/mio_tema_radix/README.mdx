...
mio_tema_radix theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of mio_tema_radix theme and run the following commands:
`nvm use`
...
`drush en mio_tema_radix -y`
`drush config-set system.theme default mio_tema_radix -y`
